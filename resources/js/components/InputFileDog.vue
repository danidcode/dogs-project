<template>
    <div class="w-100 text-center input-file">
        <img id="dog_display_uploaded" @click="fileTrigger" ref="image" class="img-custom-file img-fluid cursor-pointer"
            :src="this.current_image != null ? this.current_image : this.imageUrl" style="max-height: 40rem; max-width: 40rem;">
        <br>
        <small id="dog-lbl-valid" class="w-100 text-danger d-none">La imagen es
            requerida</small>
    </div>
    <input type="text" v-bind:value="imgBase64" class="image-upload-required" id="dog_image_uploaded" name="dog"
        value="" style="position: absolute;z-index: -1;">
    <input type="file" ref="dog_image_upload" hidden id="dog_image_upload" @change="changeImg">
    <input type="text" hidden id="dog_image_uploaded_update" @change="imgUpdate(this)" class="image-uploaded-update">

    <button type="button" @click="deleteImg" class="btn py-50 px-25" id="dog_delete_btn">
